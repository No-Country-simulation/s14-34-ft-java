...
export default function Nav() {
  ...
  const handleImageClick = () => {
    const newImage =
      selectedImage === "/mascotas/8.png"
        ? "/perfil/fotoperfil.png"
        : "/mascotas/8.png";
        ...
        <div>
          <Link href="#" className="p-2">
            <svg
              aria-hidden="true"
              role="status"
              className="inline w-8 h-8 me-3 text-color2 animate-spin"
              viewBox="0 0 100 101"
              fill="none"
...
font-medium"
        ...
        <div>{/* <Link href="#" onClick={() => signOut()}>Salir</Link> */}</div>
      </div>
    );
  }
  if (session) {
          ...
          {userData && (
            <div className="flex gap-2 justify-center justify-items-center">
              <div onClick={handleImageClick}>
                <Image
                  className="w-[60px] h-[60px] relative rounded-[200px]"
                  src={selectedImage}
                  alt="foto perfil"
                  width={600}
                  height={600}
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <Link
                        href="/dashboard"
                        className={`${
                          active ? "bg-color1 text-white" : "text-gray-900"
